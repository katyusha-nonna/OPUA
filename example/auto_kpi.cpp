...
int main()
{
	...
	OpLIntArr qk1(env, K, OpLInt(0)); // 辅助参数-季度起始月份
	OpLIntArr qk2(env, K, OpLInt(0)); // 辅助参数-季度内月份数量
	{
		OpULInt k(0);
	...
	OpFloat monUB(cfg.getFloat("AutoKPI.MonthUpperBound", OpFloat(1))); //参数-月度激励指标上限（默认0.1）
	OpFloat quarLB(cfg.getFloat("AutoKPI.QuarterLowerBound", OpFloat(0.046))); //参数-季度激励指标下限（默认0.046）
	OpFloat quarUB(cfg.getFloat("AutoKPI.QuarterUpperBound", OpFloat(1))); //参数-季度激励指标上限（默认0.08）
	OpFloat yearLB(cfg.getFloat("AutoKPI.YearLowerBound", OpFloat(0.046))); //参数-年度激励指标下限（默认0.046）
	OpFloat yearUB(cfg.getFloat("AutoKPI.YearUpperBound", OpFloat(1))); //参数-年度激励指标上限（默认1）
	...
	OpLInt lastNum(cfg.getLInt("AutoKPI.LastNum", OpLInt(0))); //参数-B-岗人数（默认0，不能高于A岗总人数，也相当于A+岗人数）
	...
	OpVarMat z05(env); // 变量数组-浮点-个人季度总分（辅助变量）
	if (isSemi)
	{
		for (OpULInt i = 0; i < N; i++)
		{
			OpVarArr tmp(env);
			for (OpULInt k = 0; k < K; k++)
				tmp.add(OpVar(env, OpVarType::Con, -Constant::Infinity, Constant::Infinity, "z05_" + std::to_string(i) + "_" + std::to_string(k)));
			z05.add(tmp);
		}
	}
	...
	OpArray<OpLEArr> s5(env, N, env, K, OpFloat(0)); // 中间变量-个人季度总分
	...
	if (isSemi)
	{
		...
		for (OpLInt i = 0; i < N; i++)
			for (OpULInt k = 0; k < K; k++)
				mdl.add(OpLinCon(env, OpFloat(0), s5[i][k] - z05[i][k], OpFloat(0)));
	}
	// 随机化
	if (isRand)
	{
